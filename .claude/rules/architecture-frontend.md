...
- **Exception assumée : `lib/utils.ts`.** Il ne fait que ré-exporter `cn()`, le helper de fusion de classes CSS du paquet `cn`, qui est une fonction pure et n'adapte donc rien. Il est là parce que c'est l'emplacement par défaut de shadcn, déclaré dans `components.json` sous `aliases.utils`. **Ne pas le déplacer** : la CLI le régénérerait à cet endroit. Les composants générés, eux, importent `cn` directement depuis le paquet `cn` : c'est la sortie actuelle de la CLI, on ne la réécrit pas. Cette exception ne s'étend à rien d'autre : aucun autre helper pur ne se range dans `lib/`, sa place est dans `logic/`.
